...
def convertTo512Bits(stringInput):
	...
	for number in byteCode:
		output+='{0:08b}'.format(ord(number))
...
if __name__ =="__main__":
	...
	parser.add_argument("-j","--job",help="Specify whether to hash a custom input or a randomly-generated input",required=True)
	parser.add_argument("-i","--input",help="Text to be hashed with sha256 or the length of the to-be-generated randomized char sequence",required=True)
	args = parser.parse_args()
	# Extract the input string
	if args.job=="custom":
		stringInput = args.input
		print("The custom string you input is: {0}\n".format(stringInput))
	elif args.job=="random":
		try:
			stringLength = int(args.input)
		except ValueError:
			print("Please provide a valid length value to randomly generate a string to be hashed\n")
			exit()
		stringInput = "".join([random.choice(string.ascii_letters+string.digits+" ") for n in range(stringLength)])
		print("The randomly-generated string is: {0}\n".format(stringInput))
	else:
		print("Please specify \"job\" as either\"custom\" or \"random\"")
		exit()
	...
	correctResult = hashlib.sha256(stringInput).hexdigest()
	...
	# Generate the .dat file for the string
	f = open("message.dat","w")
	for index in range(len(stringInput)):
		f.write("@{0} {1}\n".format("{0:x}".format(index),"{0:02x}".format(ord(stringInput[index]))))